...
if [[ ${use_dockerhub} = true ]]; then
	...
	if [[ ${is_self_signed_certificate} == true ]]; then
		...
		cert_secret_name="${cert_secret_name}" yq e  '(select(.kind == "Deployment" and .metadata.name =="kpack-controller" )| .spec.template.spec += {"volumes": [{"name": "korifi-registry-ca-cert", "secret": {"defaultMode": 420, "secretName": env(cert_secret_name)}}] })' ../dependency/kpack/release-0.16.1.yaml | yq e  '(select(.kind == "Deployment" and .metadata.name =="kpack-controller" )| .spec.template.spec.containers[0] += {"volumeMounts": [{"mountPath": "/etc/ssl/certs/registry-ca.crt", "name": "korifi-registry-ca-cert", "readOnly": true, "subPath": "ca.crt"}]})' | kubectl apply -f -
